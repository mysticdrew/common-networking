...
### Add to your project
Versions follow `{minecraft_version}-{mod_version}`, e.g. `26.2-1.1.0`.
...
public class MyPacket implements CustomPacketPayload {
    public static final CustomPacketPayload.Type<MyPacket> TYPE =
            new CustomPacketPayload.Type<>(Identifier.fromNamespaceAndPath(MOD_ID, "my_packet"));
...
```java
Network.registerPacket(MyPacket.TYPE, MyPacket.STREAM_CODEC, MyPacket::handle);
```
